...
export function WaveformCanvas() {
  ...
  useEffect(() => {
    ...
    if (!canvasContainer || !namesContainer) return;

    let syncing = false;
    const syncFromCanvas = () => {
      if (syncing) return;
      syncing = true;
      namesContainer.scrollTop = canvasContainer.scrollTop;
      syncing = false;
    };
    const syncFromNames = () => {
      if (syncing) return;
      syncing = true;
      canvasContainer.scrollTop = namesContainer.scrollTop;
      syncing = false;
    };

    canvasContainer.addEventListener('scroll', syncFromCanvas);
    namesContainer.addEventListener('scroll', syncFromNames);
    return () => {
      canvasContainer.removeEventListener('scroll', syncFromCanvas);
      namesContainer.removeEventListener('scroll', syncFromNames);
    };
  }, []);
  ...
  return (
    <div className="waveform-panel">
      ...
      <div className="waveform-content">
        {/* Signal name labels */}
        <div className="signal-names" ref={signalNamesRef}>
